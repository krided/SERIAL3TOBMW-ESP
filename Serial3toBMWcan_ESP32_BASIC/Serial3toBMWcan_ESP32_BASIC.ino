...
void handleCelLamp() {
  ...
  if (!celActive) {
    ...
    celActive = true;
    celStartTime = millis();
    CAN_msg_MPG_CEL[0] = 0x02; // light the lamp
  } else {
    // Check in loop if 3 seconds have passed
    if (millis() - celStartTime >= 3000) {
      CAN_msg_MPG_CEL[0] = 0x00; // turn off the lamp
      celActive = false;
    }
  }
}
...
void handleCelELMLamp() {
  ...
  if (!celActive) {
    ...
    celActive = true;
    celStartTime = millis();
    CAN_msg_MPG_CEL[0] = 0x12; // light the lamp
  } else {
    // Check in loop if 3 seconds have passed
    if (millis() - celStartTime >= 3000) {
      CAN_msg_MPG_CEL[0] = 0x00; // turn off the lamp
...
void handleEMLLamp() {
  ...
  if (!celActive) {
    ...
    celActive = true;
    celStartTime = millis();
    CAN_msg_MPG_CEL[0] = 0x10; // light the lamp
  } else {
    // Check in loop if 3 seconds have passed
    if (millis() - celStartTime >= 3000) {
      CAN_msg_MPG_CEL[0] = 0x00; // turn off the lamp
